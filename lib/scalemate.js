...
function Scalemate(config) {
    ...
    for (let metricName in config.metrics) {
        // Get metric by key
        let metric = config.metrics[metricName];
    ...
    co(function* () {
        try {
            ...
            for (let metric of that.metrics) {
                ...
                putMetricData(that.cloudwatch, awsMetric).then(function () {
                    // Log successful metric publish
                    log.info('scalemate', `${awsMetric.MetricData[0].Timestamp.toISOString()} ${metric.name}: ${value}`);
                }).catch(function (err) {
                    // Log exception
                    log.error('scalemate', `${awsMetric.MetricData[0].Timestamp.toISOString()}`, err);
                });
            }
        }
        catch (err) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        let handleErr = function (err) {
            ...
            if (err && err.code === 'Throttling') {
                // Log throttling
                log.warn('scalemate', `${new Date().toISOString()} Throttled by CloudWatch, retrying...`);
...
function scheduleNextRun() {
    ...
    let currentMilliseconds = nextRunTime.getTime();
    ...
    let timeUntilNextRun = nextRunTime.getTime() - currentMilliseconds;
